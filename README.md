...
- 距離最短、ルール遵守、安全性、情報信頼性、バランスの5プロファイル
- 5ルートの色分け表示、指標比較表、選択ルートの強調表示
- 道路種別、自転車設備、属性欠損を用いた暫定コストによるルート選定
- 自動テストと再現可能な確認コマンド
...
Leafletが道路GeoJSONを地図上に描画
        ↓
地図で2点選択 → 最寄りノード → NetworkXプロファイル別経路
        ↓
5種類の経路GeoJSON・距離・道路属性指標を比較表示
```
...
│  │  └─ routing.py            # プロファイル別経路・経路指標・暫定コスト
│  ├─ scripts/
...
- 道路GeoJSONを緑色で地図上に表示
- 地図を2回クリックすると、5種類の経路を色分けして表示
- 比較表の行を選ぶと、対応する経路を地図上で強調表示
- 各経路の距離、道路区間数、幹線道路率、自転車レーン率、信号数、属性不明率、プロファイル仮コストを表示
...
返却するプロファイルは次の5種類です。重みは研究途中の仮値であり、現時点では優劣や安全性を保証しません。

| プロファイル | 現段階での扱い |
|---|---|
| 距離最短 | 道路延長だけを使用する比較基準 |
| ルール遵守優先 | 明示的な通行禁止を除外し、情報不足区間を抑制 |
| 安全性優先 | 幹線道路と自転車設備のない区間を強く抑制 |
| 情報信頼性優先 | 評価用OSM属性が多く記録された区間を優先 |
| バランス型 | 道路種別、自転車設備、情報不足を組み合わせる |
...
## Phase 4の途中経過で報告できる内容

1. 距離最短だけでなく、研究目的に対応する5プロファイルを定義した。
2. 明示的な`bicycle=no`・`access=no/private`は制度対応側で通行不可として扱う。
3. 道路種別、自転車設備の有無、5属性の不明率を独立したコスト要素へ分けた。
4. NetworkXの重み付き最短路として各プロファイルを実際に計算するAPIを作成した。
5. Web画面で5ルートを色分けし、距離と道路属性指標を同条件で比較できるようにした。
6. 東京駅周辺の実データで、距離増加と幹線道路率・自転車レーン率の変化というトレードオフを確認した。
7. 現時点の重みは暫定値であり、次工程で感度分析、現地確認、複数ODペア評価を行う必要がある。
...
- 暫定重みを複数パターンで変え、経路と指標の感度を分析する
- プロファイル間で同一経路になった場合を画面上で分かりやすく示す
